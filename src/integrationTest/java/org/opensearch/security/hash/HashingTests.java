...
public class HashingTests extends RandomizedTest {
    ...
    public static String generateSCryptHash(String password, int workFactor, int resources, int parallelization, int derivedKeyLength) {
        return Password.hash(CharBuffer.wrap(password.toCharArray()))
            .with(ScryptFunction.getInstance(workFactor, resources, parallelization, derivedKeyLength))
            .getResult();
    }
    ...
    public static String generateArgon2Hash(
        ...
        return Password.hash(CharBuffer.wrap(password.toCharArray()))
            .with(Argon2Function.getInstance(memory, iterations, parallelism, length, Argon2.valueOf(type.toUpperCase()), version))
            .getResult();
    }
